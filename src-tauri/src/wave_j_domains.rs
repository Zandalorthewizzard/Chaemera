use crate::sqlite_support::{
    dyad_apps_base_directory, normalize_path, now_unix_timestamp, open_db,
    resolve_workspace_app_path, timestamp_to_rfc3339,
};
...
fn app_json(app: &DbApp, include_files: bool) -> Result<Value, String> {
    let resolved_path = resolve_workspace_app_path(&app.path)?;
