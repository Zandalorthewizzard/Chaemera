...
/**
 * Accent color configuration for action card components.
 * Maps to Tailwind color classes for border, background, and text.
 */
export type ActionAccentColor =
  | "blue"
  | "purple"
  | "violet"
  | "red"
...
const ACCENT_BORDER: Record<ActionAccentColor, string> = {
...
const ACCENT_ICON_BG: Record<ActionAccentColor, string> = {
...
const ACCENT_BADGE: Record<ActionAccentColor, string> = {
...
/**
 * Premium container for all markdown action cards.
...
/**
 * Header row for ActionCard. Contains a tinted icon circle and flexible content area.
 */
 ...
 */
export function ActionBadge({ children, color = "blue" }: ActionBadgeProps) {
  return (
    <span
...
interface ActionExpandIconProps {
  isExpanded: boolean;
}
 ...
 */
export function ActionExpandIcon({ isExpanded }: ActionExpandIconProps) {
  return (
    <ChevronRight
      size={16}
 ...
 */
export function ActionStateIndicator({
  state,
  pendingLabel,
  abortedLabel,
  finishedLabel,
}: ActionStateIndicatorProps) {
  if (state === "pending") {
    return (
 ...
 */
export function ActionFinishedIcon() {
  return (
 ...
 */
export function ActionFilePath({ path }: ActionFilePathProps) {
  if (!path) return null;
  return (
 ...
 */
export function ActionDescription({ children }: ActionDescriptionProps) {
  return (
