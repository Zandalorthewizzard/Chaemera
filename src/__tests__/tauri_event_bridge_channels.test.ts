...
describe("tauri event bridge channels", () => {
  it("marks the remaining receive-only contract channels as Tauri-supported", () => {
    ...
    expect(isTauriMigrationEventChannel("help:chat:response:chunk")).toBe(true);
    expect(isTauriMigrationEventChannel("help:chat:response:end")).toBe(true);
    expect(isTauriMigrationEventChannel("help:chat:response:error")).toBe(true);
