...
describe("paths helpers", () => {
  ...
  const originalDyadAppsDir = process.env.CHAEMERA_TAURI_DYAD_APPS_DIR;

  afterEach(() => {
    process.env.CHAEMERA_TAURI_APP_DATA_DIR = originalUserDataDir;
    process.env.CHAEMERA_TAURI_DYAD_APPS_DIR = originalDyadAppsDir;
  });

  it("uses Tauri override env vars when present", () => {
    process.env.CHAEMERA_TAURI_APP_DATA_DIR = "/tmp/chaemera-user-data";
    process.env.CHAEMERA_TAURI_DYAD_APPS_DIR = "/tmp/chaemera-apps";
    ...
    expect(getDyadAppsBaseDirectory()).toBe("/tmp/chaemera-apps");
    expect(getDyadAppPath("demo")).toBe(
      path.join("/tmp/chaemera-apps", "demo"),
    );
  });
  ...
  it("falls back to the host home directory and local userData directory", () => {
    ...
    expect(getDyadAppsBaseDirectory()).toBe(
