...
async function verifyReleaseAssets() {
  try {
    ...
    const owner = "Zandalorthewizzard";
    const repo = "Chaemera";
    ...
    const response = await fetch(allReleasesUrl, {
      headers: {
        ...
        "User-Agent": "chaemera-release-verifier",
      },
    });
    ...
    const expectedAssets = [
      `chaemera-${normalizeVersionForPlatform(version, "rpm")}-1.x86_64.rpm`,
      `chaemera-${normalizeVersionForPlatform(version, "nupkg")}-full.nupkg`,
      `chaemera-${version}.Setup.exe`,
      `chaemera-darwin-arm64-${version}.zip`,
      `chaemera-darwin-x64-${version}.zip`,
      `chaemera_${normalizeVersionForPlatform(version, "deb")}_amd64.deb`,
      `chaemera_${version}_x86_64.AppImage`,
      "RELEASES",
    ];
